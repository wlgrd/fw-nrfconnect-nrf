...
#define prod_assert_equal(a, b, err, msg) if(a != b) { \
                                        printk("Error at %s, line %d -  %s\r\n", __func__, __LINE__, msg); \
                                        return err; \
                                        }

#define prod_assert_not_equal(a, b, err, msg) if(a == b) { \
                                        printk("Error at %s, line %d -  %s\r\n", __func__, __LINE__, msg); \
                                        return err; \
                                        }
#define prod_assert_not_null(a, err, msg) if(a == NULL) { \
                                        printk("Error at %s, line %d -  %s\r\n", __func__, __LINE__, msg); \
                                        return err; \
                                        }
...
static int pca20035_test_button(void)
{
	...
	printk("timeout: %d \n", timeout);
        prod_assert_not_equal(timeout, button_test_timeout, -ETIMEDOUT, "Button test timed out");
	dk_set_leds(DK_NO_LEDS_MSK);
        return 0;
}
...
static int pca20035_test_buzzer(void)
{
        ...
        err_code = pwm_pin_set_usec(dev, BUZZER_PIN,
                                period, duty_cycle);
